...
use rand::{SeedableRng, rngs::{SmallRng, ThreadRng}};
use std::fs;
use docx_rust::{
    document::Paragraph,
    Docx,
...
const WORDLIST_URL: &str = "https://raw.githubusercontent.com/dwyl/english-words/master/words_alpha.txt";
...
fn main() {
    ...
    let content = get(WORDLIST_URL).call().expect("Failed to download wordlist").into_string().expect("Failed to parse wordlist to string");
    let words: Vec<&str> = content.lines().map(|x| x.trim()).filter(|x| x.len() >= MIN_WORD_SIZE && x.len() <= MAX_WORD_SIZE).collect();
    let mut rng_cheap = SmallRng::from_rng(ThreadRng::default()).expect("Failed get random number generator");
    ...
    for group in GROUPS {
        ...
        if fs::metadata(&file_name).is_ok() {
            ...
            let file_content = match docx.parse() {
                Ok(file_content_result) => {
                    file_content_result.document.body.text()
                },
                Err(file_content_result) => {
                    println!("❌ File {} does not match {} ({:?})", file_name, hash, file_content_result);
                    continue;
                }
            };
            ...
            let random_words = match words.get(0..16) {
                Some(random_words_result) => {
                    random_words_result
                },
                None => {
                    println!("❌ File {} does not match {} (Unable to parse random word key)", file_name, hash);
                    continue;
                }
            };
                        
            let hash_words = match words.get(16..) {
                Some(hash_words_result) => {
                    hash_words_result
                },
                None => {
                    println!("❌ File {} does not match {} (Unable to parse encoding words)", file_name, hash);
                    continue;
                }
            };
            ...
            for hash_word in hash_words {
                let index = match random_words.iter().position(|x| x == hash_word) {
                    ...
                    None => {
                        println!("❌ File {} does not match {} (Unable to decode word: {})", file_name, hash, hash_word);
                        continue;
                    }
                };
            ...
            let random_words: Vec<String> = indices.iter().map(|&i| &words[i]).map(|i| i.to_string()).collect();
        
            let mut file_content = random_words.join(" ");
            for hex_char in hash.chars() {
                let word = random_words.get(u8::from_str_radix(&hex_char.to_string(), 16).expect("Failed to parse hex to byte") as usize).expect("Failed to encode in random word");
