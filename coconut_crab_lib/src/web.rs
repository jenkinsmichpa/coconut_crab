...
pub mod client {
    ...
    pub fn web_get_recv_bytes(url: &String, verify_server: &bool) -> Option<Vec<u8>> {
    ...
    pub fn web_post_send_json_recv_text<T: serde::Serialize>(url: &String, json: &T, verify_server: &bool) -> Option<String> {
...
pub mod validate {
    ...
    lazy_static! {
        ...
        static ref HOSTNAME_REGEX: Regex = Regex::new(r"^[0-9A-Za-z.\-]{0,32}$").expect("Invalid Regex");
        ...
        static ref CODE_REGEX: Regex = Regex::new(r"^[[:alnum:]]{4}-[[:alnum:]]{4}-[[:alnum:]]{4}-[[:alnum:]]{4}$").expect("Invalid Regex");
    }
    ...
    pub fn validate_hostname(hostname: &String) -> bool {
        HOSTNAME_REGEX.is_match(hostname)
    }
    
    pub fn validate_proof(proof: &String) -> bool {
    ...
    pub fn validate_code(code: &String) -> bool {
...
mod reqwest_client {
    ...
    fn create_client_reqwest(verify_server: &bool) -> reqwest::blocking::Client {
        ...
        if *verify_server {
            ...
            let ca_cert = reqwest::Certificate::from_pem(&get_ca_public_key()).expect("CA certificate not valid");
    ...
    pub fn web_get_recv_bytes_reqwest(url: &String, verify_server: &bool, initial_retry_wait: &u64, retries: &u8) -> Option<Vec<u8>> {
...
mod ureq_client {
    ...
    fn create_client_ureq(verify_server: &bool) -> Agent {
        ...
        if *verify_server {
            ...
            tls_connector = TlsConnector::builder()
