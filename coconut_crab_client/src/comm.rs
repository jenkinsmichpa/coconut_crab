...
pub fn register(server_fqdn: &str, port: &u16, status: &Status, secret: &str, https: &bool, verify_server: &bool) {
    ...
    let registration = Registration {id: status.id.clone(), hostname: status.hostname.clone(), proof: create_proof(&mut proof_source, secret)};
    debug!("Registration data: {:?}", registration);
    let content = web_post_send_json_recv_text(&url, &registration, verify_server).expect("Unable to get content");
    debug!("Response content text: {}", content);
    assert_eq!(content, "Success");
}

pub fn upload_sym_key(server_fqdn: &str, port: &u16, status: &Status, secret: &str, https: &bool, verify_server: &bool) {
    let identifier = if *https { "https" } else { "http" };
    let url = format!("{}://{}:{}/upload-sym-key", identifier, server_fqdn, port);
    debug!("Symmetric key upload URL: {}", url);
    let mut proof_source = [status.id.as_bytes(), status.asymmetrically_encrypted_symmetric_key.as_bytes()].concat();
    debug!("Proof source: {:?}", proof_source);
    let uploadsymkey = UploadSymKey {id: status.id.clone(), key: status.asymmetrically_encrypted_symmetric_key.clone(), proof: create_proof(&mut proof_source, secret)};
    debug!("Symmetric key upload data: {:?}", uploadsymkey);
    let content = web_post_send_json_recv_text(&url, &uploadsymkey, verify_server).expect("Unable to get content");
    debug!("Response content text: {}", content);
    assert_eq!(content, "Success");
    info!("Successfully uploaded symmetric key");
}

pub fn announce_completion(server_fqdn: &str, port: &u16, status: &Status, secret: &str, https: &bool, verify_server: &bool) {
    let identifier = if *https { "https" } else { "http" };
    ...
    debug!("Proof source: {:?}", proof_source);
    let announcecompletion = AnnounceCompletion {id: status.id.clone(), proof: create_proof(&mut proof_source, secret)};
    debug!("Completion announcement data: {:?}", announcecompletion);
    let content = web_post_send_json_recv_text(&url, &announcecompletion, verify_server).expect("Unable to get content");
    debug!("Response content text: {}", content);
    assert_eq!(content, "Success");
    info!("Successfully announced completion");
}

fn download_sym_key(server_fqdn: &str, port: &u16, status: &Status, code: &str, secret: &str, https: &bool, verify_server: &bool) -> Option<[u8; 32]> {
    let identifier = if *https { "https" } else { "http" };
    ...
    debug!("Proof source: {:?}", proof_source);
    let downloadsymkey = DownloadSymKey {id: status.id.clone(), code: code.to_string(), proof: create_proof(&mut proof_source, secret)};
    debug!("Symmetric key download data: {:?}", downloadsymkey);
    let content = web_post_send_json_recv_text(&url, &downloadsymkey, verify_server).expect("Unable to get content");
    debug!("Response content text: {}", content);
...
pub fn get_sym_key(server_fqdn: &str, server_port: &u16, status: &Status, code: &str, preshared_secret: &str, https: &bool, verify_server: &bool) -> Option<[u8; 32]> {
    match download_sym_key(server_fqdn, server_port, status, code, preshared_secret, https, verify_server) {
        Some(sym_key_result) => {
            debug!("Server sent back a valid symmetric key: {:?}", sym_key_result);
            let decrypt_id_attempt = decrypt_string(&status.symmetrically_encrypted_id, &sym_key_result, &status.symmetrically_encrypted_id_nonce);
            debug!("Comparing server provided key decrypted ID ({}) to known ID ({})", decrypt_id_attempt, status.id);
