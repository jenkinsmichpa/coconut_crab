...
use chacha20::{ cipher::{ KeyIvInit, StreamCipher, StreamCipherSeek }, ChaCha20 };
    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            if let Some(original_extension) = file_path.as_ref().extension() {
                ...
                encrypted_file_path.set_extension(
                    format!("{}.{}", original_extension.to_string_lossy(), nonce_file_extension)
                );
            } else {
...
fn apply_chacha(
    ...
    let source_file_data = match
        get_file_data(source_file_path, &(MAX_IN_MEMORY_CHACHA_SIZE_MB * 1024 * 1024))
    {
